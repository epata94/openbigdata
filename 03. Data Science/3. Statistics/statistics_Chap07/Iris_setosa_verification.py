...
sample_data_index_list = [60,61,62,63,64,65]
...
for pridicted_value in y_predicted_rounded:
    if pridicted_value > 0.5:
        print("%d번째 샘플링 데이터 예측 결과 %f : Setosa 확실"%(index,pridicted_value))
    else:
        print("%d번째 샘플링 데이터 예측 결과 %f : Setosa 아닌 다른 품종"%(index,pridicted_value))
    index+=1
